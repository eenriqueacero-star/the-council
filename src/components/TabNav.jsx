...
import { MessageSquare, Crown, Briefcase, Coins, Radar, Map } from 'lucide-react';
...
export default function TabNav({ tab, setTab }) {
  return (
    <div className="boot mt-6 grid grid-cols-3 gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/10 w-full" style={{ animationDelay: '.16s' }}>
      {TABS.map(t => {
        const Icon = t.icon;
        const sel = tab === t.id;
        return (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            className="px-3 py-2 rounded-lg text-[12px] font-semibold flex items-center justify-center gap-1.5 transition-all whitespace-nowrap">
