...
import KnowledgeNetwork from './network/KnowledgeNetwork.jsx';
import NodeInfoPanel from './network/NodeInfoPanel.jsx';
import AgentChatSheet from './network/AgentChatSheet.jsx';
...
export default function CouncilTab({ account, acct, acctHoldings, positionsLine, flagApiDown, running, setRunning, ticker, setTicker, capital, setCapital, active, setActive, agentState, setAgentState, synthesis, setSynthesis, dark }) {
  ...
  const [selectedNode, setSelectedNode] = useState(null); // tapped node/edge in the knowledge network
  const [chatAgentId, setChatAgentId] = useState(null); // agentId with an open "Talk to" chat sheet
...
BUY = approved entry. WATCH = wait for better setup. SKIP = council rejects this trade — do not enter.`;
  ...
  return (
    <div style={{ marginTop: 8 }}>
      {/* Living Knowledge Network — replaces the 3D chamber. Grows organically from
          real agent_feed / agent_memory / council_reports activity; pulses only fire
          on real Firestore events, never decoratively. */}
      <div style={{ marginBottom: 20 }}>
        <KnowledgeNetwork
          dark={dark}
          uid={uid}
          holdings={acctHoldings?.length ? acctHoldings : (acct.holdings || [])}
      ...
      <AnimatePresence>
        {selectedNode && (
          <NodeInfoPanel
            ...
            onOpenChat={agentId => { setSelectedNode(null); setChatAgentId(agentId); }}
          />
        )}
      </AnimatePresence>
      <AnimatePresence>
